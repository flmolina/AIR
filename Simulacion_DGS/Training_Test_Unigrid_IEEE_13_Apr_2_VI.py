...
def getatri(V,I,Tsi,Tfal,Tout):
    ...
    des[0,3:6]=temp[0,3:6]
    des[0,6:9]=temp1[0,0:3]
    des[0,9:12]=temp1[0,3:6]
    des[0,12:15]=Vmag[0:3]
    des[0,15:18]=Vang[0:3]
    des[0,18:21]=Imag[0:3]
    des[0,21:24]=Iang[0:3]
    des[0,24:27]=Vmag[3:6]
    des[0,27:30]=Vang[3:6]
    des[0,30:33]=Imag[3:6]
    des[0,33:36]=Iang[3:6]
    # des[0,44]=np.abs(Vsecpre[0])
    # des[0,45]=np.angle(Vsecpre[0])
    # des[0,46]=np.abs(Vsecpre[2])
    # des[0,47]=np.angle(Vsecpre[2])
    # des[0,48]=np.abs(Isecpre[0])
    # des[0,49]=np.angle(Isecpre[0])
    # des[0,50]=np.abs(Isecpre[2])
    # des[0,51]=np.angle(Isecpre[2])
    
    des1F=np.zeros((1,36))
    # des1F[0,0:8]=des[0,0:8]-des[0,44:52]
    # des1F[0,8:11]=des[0,11:14]-des[0,8:11]
    # des1F[0,11:14]=des[0,17:20]-des[0,14:17]
    # des1F[0,14:26]=des[0,32:44]-des[0,20:32]
    # des1F[0,26]=des[0,0]-des[0,2]
    # des1F[0,27]=des[0,1]-des[0,3]
    # des1F[0,28]=des[0,4]-des[0,6]
    # des1F[0,29]=des[0,5]-des[0,7]
    # des1F[0,30]=des[0,0]/des[0,2]
    # des1F[0,31]=des[0,1]/des[0,3]
    # des1F[0,32]=des[0,4]/des[0,6]
    # des1F[0,33]=des[0,5]/des[0,7]
    # des1F[0,34]=des[0,38]
    # des1F[0,35]=des[0,39]
    # des1F[0,36]=des[0,40]
    ...
    des1F[0,0:3]=des1F[0,0:3]*1000000
    des1F[0,3:6]=des[0,3:6]*1000000
    #############################################
    
    ###  Información de impedancia #####
    des1F[0,6:9]=des[0,6:9]
    des1F[0,9:12]=des[0,9:12]
    ###################################
    
    ###Información  de magnitudes y angulos voltajes y corriente falla #######
    des1F[0,12:15]=des[0,12:15]*1000
    des1F[0,15:18]=des[0,15:18]
    des1F[0,18:21]=des[0,18:21]*1000
    des1F[0,21:24]=des[0,21:24]
    ################################################
    
    ###Información  de magnitudes y angulos voltajes y corriente prefalla #######
    des1F[0,24:27]=des[0,24:27]*1000
    des1F[0,27:30]=des[0,27:30]
    des1F[0,30:33]=des[0,30:33]*1000
    des1F[0,33:36]=des[0,33:36]
...
def Base_data(Info,TIIE,TXs):
    ...
    Info1X=scaler.fit_transform(Info1[:,0:36])
    Info1Y=Info1[:,36]
    #Splited
    ...
    #Save Test data base
    archivo=open(('C:\\Juan_python\IUnigrid_TestV2_App2_37F_%s.csv' %(str(TXs))),'w')
    ...
    archivo=open(('C:\\Juan_python\IUnigrid_TrainV2_App2_37F_%s.csv' %(str(TXs))),'w')
...
VecW=[-30,-20,-10,0,10,20,30]
VecWFal=[-30,-20,-10,0,10,20,30]
#VecW=[-40,-30,-20,-10,0,10,20,30,40]
#VecWFal=[-40,-30,-20,-10,0,10,20,30,40]
#Scenario information
dirto=pd.read_csv("Escenarios_Falla_Unigrid_falla.csv",sep=",")
Xf_d=dirto.iloc[0:np.size(dirto,0),:].values
dirto=pd.read_csv("Escenarios_Falla_Unigrid_No_falla_V1.csv",sep=",")
...
direc=ls2("D:\CSV_digsilent_Falla_Unigrid")
Tar=len(direc)
direcnf=ls2("D:\CSV_digsilent_No_Falla_Unigrid_V1")
Tarnf=len(direcnf)



# Este proceso se debe hacer para cada IED
datos=np.zeros((TXs*2,100000,37))
...
#  if ky>=2:  
    ...
    for k in range(Tarnf):
        ...
        ASD=("D:\CSV_digsilent_No_Falla_Unigrid_V1//")      
        AS=ASD+direcnf1
        ...
        for kl in range(Tarnf):
            ...
            X1rand=fid1.iloc[2:771,:].values
            Rerd1 ,Rerd2=get_measures(X1,Xd1,Xs[ky,:])
            if Xf_dnf[int(direcnf1[9:int(len(direcnf1)-4)]),3]<7 and Xf_dnf[int(direcrand[9:int(len(direcrand)-4)]),3]<7:
               ...
               for hj in range(len(VecW)):
                   datos[ky*2][gh,0:36]=getatri(Re1[:,0:3],Re1[:,3:6],0.1,0.05,VecW[hj])
                   datos[ky*2][gh,36]=0
                   gh=gh+1
               for hj in range(len(VecW)):
                   datos[2*ky+1][gh1,0:36]=getatri(Re2[:,0:3],Re2[:,3:6],0.1,0.05,VecW[hj])
                   datos[ky*2+1][gh1,36]=0
                   gh1=gh1+1
            elif Xf_dnf[int(direcrand[9:int(len(direcrand)-4)]),3]>9 or Xf_dnf[int(direcnf1[9:int(len(direcnf1)-4)]),3]>9:
                Re1[round(np.size(X1,0)/2):np.size(X1,0),:]=Rerd1[round(np.size(X1,0)/2):np.size(X1,0),:]
                Re2[round(np.size(X1,0)/2):np.size(X1,0),:]=Rerd2[round(np.size(X1,0)/2):np.size(X1,0),:]
                for hj in range(len(VecW)):
                    datos[ky*2][gh,0:36]=getatri(Re1[:,0:3],Re1[:,3:6],0.1,0.05,VecW[hj])
    ...
    for k in range(Tar):
        ...
        ASD=("D:\CSV_digsilent_Falla_Unigrid//")
        AS=ASD+direc1
        fid=pd.read_csv(AS,header=None,sep=";")
        Xd1=fid.iloc[0,:].values
        X1=fid.iloc[2:771,:].values
        ...
        if ky==0:
          if (Xss=="650" and Xss1=="632") or (Xss=="633" and Xss1=="634") or (Xss=="632" and Xss1=="671"):
             ...
             for hj in range(len(VecWFal)):
                datos[ky*2][gh,0:36]=getatri(Re1[:,0:3],Re1[:,3:6],0.1,0.05,VecWFal[hj])
                datos[ky*2][gh,36]=1
                gh=gh+1
             for hj in range(len(VecWFal)):
                datos[2*ky+1][gh1,0:36]=getatri(Re2[:,0:3],Re2[:,3:6],0.1,0.05,VecWFal[hj])
                datos[ky*2+1][gh1,36]=1
                gh1=gh1+1
          else:
             Re1 ,Re2=get_measures(X1,Xd1,Xs[ky,:])
